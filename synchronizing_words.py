...
def DFA_random_generator(num_states, num_alphabet):
...
def CNF_gen(d, length, solver = 'z3'):
...
def main():
    ...
    if (sys.argv[1] == '--input'):
        ...
        solution = SAT_based(d)
        print(solution)

    if (sys.argv[1] == '--random'):

        d = DFA_random_generator(int(sys.argv[2]), int(sys.argv[3]))
        solution = SAT_based(d)
        print(solution)
